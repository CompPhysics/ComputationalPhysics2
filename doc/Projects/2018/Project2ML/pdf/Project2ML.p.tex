...
a restricted Boltzmann machine (RBM) was semented by G. Carleo and
M. Troyer, Science Vol. 355, Issue 6325, pp. 602-606 (2017). They
...
quanutm mechanics we will in this project apply it to systems of
moving particles. Thus, while the mentioned article looks to model
discrete spin values of "up" and "down", in this project we aim to
model the continuous values of particle positions.
...
In these exercises we will deal only with a system of
...
other hidden. These type of networks constitue the building blocks of
the deep belief networks, which some might have heard of. The RBM is a
...
Some might have heard that neural networks are referred to as falling
...
\begin{align}
	Z = \int \int \frac{1}{Z} e^{-\frac{1}{T_0}E(\mathbf{x},\mathbf{h})} \diff \mathbf{x} \diff \mathbf{h}
\end{align}
...
\begin{align}
	E(\mathbf{V}, \mathbf{H})= \frac{||\mathbf{X} - \mathbf{a}||^2}{2\sigma^2} - \mathbf{b}^T \mathbf{H} - \frac{\mathbf{X}^T \mathbf{W} \mathbf{H}}{\sigma^2}
\end{align}
...
it's hard, if possible, to determine them analytically. Thus
...
\begin{enumerate}
 ...
 \item \href{{https://deeplearning4j.org/restrictedboltzmannmachine}}{See notes on Deep Learning}

 \item \href{{https://www.cs.toronto.edu/~hinton/absps/guideTR.pdf}}{More notes on Deep learning}
\end{enumerate}
