...
In order to set up the two-body matrix elements, we need to define the so-called direct and exchange matrix elements. This leads what we call an anti-symmetrized matrix element.
...
\langle pq|\hat{v}|sr\rangle = \int \psi_{p}^*(x_i)\psi_{q}^*(x_j)
  ...
  dx_idx_j,  
\]
which defines the exchange element. The variables $pqrs$ define all the single-particle quantum numbers. In our case these are $n_x$, $n_y$, spin and its projections and the single-particle energies.

These matrix elements are defined in terms of two-body quantum numbers. Which quantum numbers are conserved?
What is the total spin 
...
\langle pq|\hat{v}|rs\rangle =  \int \psi_{p}^*(x_i)\psi_{q}^*(x_j)V(r_{ij})\psi_{r}(x_i)\psi_{s}(x_j)
    dx_idx_j,
\]
and then assemble the direct and exchange terms in order to construct the anti-symmetrized matrix elements to be used in the Hartree-Fock calculations. The integral is four-dimensional but can be simplified. Your task is to figure out how. You should also consider whether it is appropriate to store the matrix elements for the Hartree-Fock calculations. 
...
\sum_{a=1}^N\sum_{\beta\delta} C^*_{a\beta}C_{a\delta}\langle \alpha\beta|\hat{v}|\gamma\delta\rangle_{AS},
\end{equation*}
